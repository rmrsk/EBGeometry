...
This is the companion to [`Examples/ClosestPointSFC`](../ClosestPointSFC/README.md): it searches the
same 100,000-point cloud with the same 1,000 queries and compares the same four build strategies
-- **Morton (SFC)**, **TopDown centroid**, **Midpoint**, and **SAH** -- against a brute-force scan.
The difference is the **construction path**. Where `ClosestPointSFC` forms the `PointAoSoA<T, size_t, W>`
groups up front (Morton-sorting the cloud and chunking it into groups) and builds a BVH over those
pre-formed groups, this example builds the tree the way the library's own `TriMeshSDF` does:
...
Letting the partitioner see all 100,000 points -- instead of a coarser set of ~25,000 pre-formed
groups -- builds a **higher-quality tree**: for the top-down strategies it roughly halves the leaves
visited per query relative to the corresponding row of `ClosestPointSFC`, so queries are faster, at
the cost of a slower build (the tree is over `W`x as many primitives). Run both examples and compare
their `SAH` rows to see the trade-off.

The four strategies are not all built the same way, which the output makes visible:
...
* **Morton (SFC)** is bottom-up (`TreeBVH::bottomUpSortAndPartition<SFC::Morton>()`), which builds
  fixed `K`-primitive leaves and ignores `maxLeafGroups`. With `K == W` that is one group per leaf,
  so the tree is much deeper and queries visit many more leaves -- via this path the space-filling
  curve is a poor match for the `PointAoSoA` grouping, and the top-down partitioners win clearly.
  (`ClosestPointSFC`'s **Morton (SFC)** row is a different construction -- `PackedBVH`'s direct SFC
  constructor with a tunable leaf size -- so the two Morton rows are not directly comparable.)
...
so results are reproducible on a given machine) and prints a short header followed by one table row
per strategy giving build time, average query time, speedup over brute force, average leaf visits per
...
* **Leaf visits are the payoff.** Compared with the matching row of `ClosestPointSFC` (same query
  code, same leaf primitive), the top-down builds here visit fewer leaves per query, because the
  partitioner worked over the full cloud rather than pre-formed groups -- that is where the
...
* **Leaf size is tuned via `maxLeafGroups` in `main.cpp`** for the top-down strategies. A leaf-size
  ...
  8 or 32 to see the effect. (Morton, being bottom-up, is unaffected.)
