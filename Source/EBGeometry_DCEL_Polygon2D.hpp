...
namespace DCEL {
  ...
  /*!
    ...
    @details This class is required for determining whether or not a 3D point projected to the plane of an N-sided polygon lies inside or outside the polygon face. To do
    this we compute the 2D embedding of the polygon face, reducing the problem to a tractable dimension where we can use well-tested algorithm. The 2D embedding of a polygon
    occurs by taking a set of 3D points and a corresponding normal vector, and projecting those points along one of the 3D Cartesian axes such that the polygon has the largest
    area. In essence, we simply find the direction with the smallest normal vector component and ignore that. Once the 2D embedding is computed, we can use well-known algorithms
    for checking if a point lies inside or outside. The supported algorithms are 1) The winding number algorithm (computing the winding number), 2) Computing the subtended 
    angle of the point with the edges of the polygon (sums to 360 degrees if the point is inside), or computing the crossing number which checks how many times a ray cast
    from the point crosses the edges of the polygon. 
  */
  template <class T>
  class Polygon2D {
  public:
    ...
    /*!
      @brief Supported algorithms for performing inside/outside tests when checking if a point projects to the inside or outside of a polygon face. 
    */
    ...
    */
    ...
    /*!
      @brief Check if a point is inside a 2D polygon, using the winding number algorithm
      @param[in] a_point 3D point coordinates
      @return Returns true if the 3D point projects to the inside of the 2D polygon
    */
    inline
    ...
    */
    ...
    /*!
      @brief Check if a point is inside a 2D polygon, by computing the number of times a ray crosses the polygon edges. 
      @param[in] a_point 3D point coordinates
      @return Returns true if the 3D point projects to the inside of the 2D polygon
    */
    inline
  ...
  private:
    ...
    */
    ...
    /*!
      @brief Project a 3D point onto the 2D polygon plane (this ignores one of the vector components)
    ...
    */
    ...
    /*!
      @brief Define function. This find the direction to ignore and then computes the 2D points.
