...
namespace DCEL {
  ...
  /*!
    @brief Class which represents a half-edge in a double-edge connected list (DCEL). 
    @details This class is used in DCEL functionality which stores polygonal surfaces in a mesh. The information contain in an EdgeT object
    contains the necessary object for logically circulating the inside of a polygon face. This means that a polygon face has a double-connected
    list of half-edges which circulate the interior of the face. The EdgeT object is such a half-edge; it represents the outgoing half-edge from a vertex,
    located such that it can be logically represented as a half edge on the "inside" of a polygon face. It contains pointers to the polygon face, next half edge, 
    and the previous half edge. It also contains a pointer to the "pair" half edge, i.e. the corresponding half-edge on the other face that shares this edge.
    Since this class is used with DCEL functionality and signed distance fields, this class also has a signed distance function and thus a "normal vector". For
    numericaly efficiency, some extra storage is also allocated (such as the vector between the starting vertex and the end vertex). 
    @note The normal vector is outgoing, i.e. a point x is "outside" if the dot product between n and (x - x0) is positive. 
  */
  template<class T>
  class EdgeT {
  public:
    ...
    */    
    ...
    /*!
      @brief Default constructor. Sets all pointers to zero and vectors to zero vectors
    */
    EdgeT();
    ...
    */
    ...
    /*!
      @brief Partial constructor. Calls the default constructor but sets the starting vertex.
    ...
    */    
    ...
    /*!
      ...
      @details This routine will check if the input point projects to the edge or one of the vertices. If it projectes
      to one of the vertices we compute the signed distance to the corresponding vertex. Otherwise we compute the projection
      to the edge and compute the sign from the normal vector. 
    */
    inline
    ...
    /*!
      @brief Get the signed distance to this half edge
      @details This routine will check if the input point projects to the edge or one of the vertices. If it projectes
      to one of the vertices we compute the squared distance to the corresponding vertex. Otherwise we compute the squared distance of the projection
      to the edge. This is faster than signedDistance()
    */
    inline
  ...
  protected:
    ...
    /*!
      ...
      @details Computed in computeNormal which sets the normal vector to be the average of the normal vector of the connected faces
    */
    Vec3 m_normal;

    /*!
      @brief Vector from the starting vertex to the end vertex. Exists for performance reasons.
    */
    Vec3 m_x2x1;
    ...
    */
    ...
    /*!
      ...
      @details This function parametrizes the edge as x(t) = x0 + (x1-x0)*t and returns where on the this edge the point a_x0 projects. If projects onto the edge if
      t = [0,1] and to one of the start/end vertices otherwise. 
    */
    inline
    ...
    */
    ...
    /*!
      ...
      @details This computes the vector m_x2x1 (vector from starting vertex to end vertex) and the inverse length squared.
    */
    inline
