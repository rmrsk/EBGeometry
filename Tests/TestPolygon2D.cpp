...
using Algo = Poly::InsideOutsideAlgorithm;

const std::array<Algo, 3> allAlgorithms = {Algo::WindingNumber, Algo::CrossingNumber, Algo::SubtendedAngle};
...
TEST_CASE("Polygon2D: a point clearly inside a square reads as inside for every algorithm", "[Polygon2D]")
{
  const Poly square = unitSquare();
...
TEST_CASE("Polygon2D: a point clearly outside a square reads as outside for every algorithm", "[Polygon2D]")
{
  const Poly square = unitSquare();
...
TEST_CASE("Polygon2D: containment ignores the coordinate normal to the polygon's plane", "[Polygon2D]")
{
  const Poly square = unitSquare();
...
TEST_CASE("Polygon2D: dedicated per-algorithm methods agree with isPointInside's dispatch", "[Polygon2D]")
{
  const Poly square = unitSquare();

  for (const Vec3 p : {Vec3(0.5, 0.5, 0), Vec3(2, 2, 0)}) {
    REQUIRE(square.isPointInside(p, Algo::WindingNumber) == square.isPointInsidePolygonWindingNumber(p));
    REQUIRE(square.isPointInside(p, Algo::CrossingNumber) == square.isPointInsidePolygonCrossingNumber(p));
    REQUIRE(square.isPointInside(p, Algo::SubtendedAngle) == square.isPointInsidePolygonSubtend(p));
  }
}

TEST_CASE("Polygon2D: a concave polygon correctly excludes points in its notch, for every algorithm", "[Polygon2D]")
{
  const Poly lPolygon = lShape();
...
TEST_CASE("Polygon2D: works for a triangle embedded in a non-axis-aligned plane", "[Polygon2D]")
{
  ...
  const Poly              triangle(Vec3(1, 1, 1), points);
...
TEST_CASE("Polygon2D: the three algorithms agree with each other over a grid of query points", "[Polygon2D]")
{
  const Poly lPolygon = lShape();
  ...
  for (int i = -2; i <= 6; i++) {
    for (int j = -2; j <= 6; j++) {
      const Vec3 p(0.25 * i + 0.07, 0.25 * j + 0.11, 0);

      const bool winding  = lPolygon.isPointInside(p, Algo::WindingNumber);
      const bool crossing = lPolygon.isPointInside(p, Algo::CrossingNumber);
      const bool subtend  = lPolygon.isPointInside(p, Algo::SubtendedAngle);
